...
// Initialize stock for all products (3 each) - ALWAYS RESET ON REFRESH
function initStock() {
    const products = document.querySelectorAll('.product-card[data-product-id]');
    
    // ALWAYS reset to 3 on every page load/refresh
...
function updateCartDisplay() {
    ...
    if (cart.length === 0) {
        cartItemsContainer.innerHTML = '<table><td colspan="5" class="empty-cart-message"><i class="fas fa-shopping-cart"></i><p>Your cart is empty</p></td></tr>';
    } else {
        cart.forEach((item, index) => {
            ...
            row.innerHTML = `
                ...
                <td>€${(item.price * item.quantity).toFixed(2)}</div></td>
...
function processReturn(confirmationCode) {
...
document.addEventListener('DOMContentLoaded', function() {
    initStock();  // Stock resets to 3 on every refresh
    loadCart();   // Cart clears on every refresh
    ...
    if (submitReturnBtn) {
        submitReturnBtn.addEventListener('click', function() {
            ...
            const result = processReturn(returnCode);
